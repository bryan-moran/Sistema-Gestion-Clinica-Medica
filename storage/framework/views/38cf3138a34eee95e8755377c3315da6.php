...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="<?php echo e($item['href']); ?>" class="<?php echo e($item['id'] === 'pacientes' ? 'active' : ''); ?>">
    ...
    <div class="content">
        <div class="page-header">
            <div class="page-header-left">
                <h1><?php echo icon('users', 24); ?> Pacientes</h1>
                <p>Listado de pacientes registrados. Haz clic en cualquier paciente para ver y editar sus datos.</p>
            </div>
        </div>
        ...
        <div class="table-card">
            <div class="table-card-header">
                ...
                <span class="table-info" id="tableInfo">Mostrando <?php echo e(count($pacientes)); ?> pacientes</span>
            </div>
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th>Paciente</th><th>DUI</th><th>Teléfono</th><th>Edad</th>
                        </tr>
                    </thead>
                    <tbody id="pacientesBody">
                        <?php $__currentLoopData = $pacientes; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $paciente): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr onclick="verPaciente('<?php echo e($paciente->dui); ?>')" style="cursor:pointer;" data-nombre="<?php echo e(strtolower($paciente->nombre.' '.$paciente->apellido)); ?>" data-dui="<?php echo e($paciente->dui); ?>" data-telefono="<?php echo e($paciente->telefono); ?>">
                            <td>
                                <div class="td-paciente">
                                    <div class="td-avatar" style="background:<?php echo e(getInitialsColor($paciente->nombre)); ?>20;color:<?php echo e(getInitialsColor($paciente->nombre)); ?>;">
                                        <?php echo e(strtoupper(substr($paciente->nombre, 0, 1))); ?><?php echo e(strtoupper(substr($paciente->apellido, 0, 1))); ?>

                                    </div>
                                    <div>
                                        <div class="td-nombre"><?php echo e($paciente->nombre); ?> <?php echo e($paciente->apellido); ?></div>
                                    </div>
                                </div>
                            </td>
                            <td><?php echo e($paciente->dui); ?></td>
                            <td><?php echo e($paciente->telefono ?? 'N/A'); ?></td>
                            <td><?php echo e($paciente->edad ?? 'N/A'); ?> años</td>
                        </tr>
                        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                    </tbody>
                </table>
                <div id="emptyState" class="empty-table" style="display:none; padding:60px;text-align:center;">
                    <?php echo icon('users',48); ?>
...
<div class="modal-overlay" id="modalPaciente">
    <div class="modal">
        <div class="modal-header">
            <h2><?php echo icon('users',18); ?> <span id="modalTitle">Datos del Paciente</span></h2>
            <button class="modal-close" id="closeModalPaciente"><?php echo icon('x',20); ?></button>
        </div>
        <div class="modal-body">
            <form id="formPaciente">
                <input type="hidden" id="edit_dui_original">
                <div class="form-row">
                    <div class="form-group"><label>Nombres</label><input type="text" id="edit_nombre" placeholder="Nombres"></div>
                    <div class="form-group"><label>Apellidos</label><input type="text" id="edit_apellido" placeholder="Apellidos"></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label>DUI</label><input type="text" id="edit_dui" placeholder="00000000-0"></div>
                    <div class="form-group"><label>Teléfono</label><input type="text" id="edit_telefono" placeholder="Teléfono"></div>
                </div>
                <div class="form-row">
                    <div class="form-group"><label>Edad</label><input type="number" id="edit_edad" placeholder="Edad" min="0" max="120"></div>
                    <div class="form-group"><label>Fecha de nacimiento</label><input type="date" id="edit_fecha_nacimiento"></div>
                </div>
                <div class="form-row full">
                    <div class="form-group"><label>Dirección</label><textarea id="edit_direccion" rows="2" placeholder="Dirección completa"></textarea></div>
                </div>
            </form>
        </div>
        <div class="modal-footer">
            <button class="btn btn-ghost" id="cancelModalPaciente">Cancelar</button>
            <button class="btn btn-primary" id="guardarPaciente"><?php echo icon('check',16); ?> Guardar cambios</button>
        </div>
    </div>
</div>
...
function verPaciente(dui) {
    // Cargar datos del paciente mediante AJAX
...
document.getElementById('guardarPaciente').addEventListener('click', function() {
    const formData = {
        ...
        dui: document.getElementById('edit_dui').value,
...
document.getElementById('searchInput').addEventListener('input', function() {
    ...
    rows.forEach(row => {
        ...
        if (nombre.includes(busqueda) || dui.includes(busqueda) || telefono.includes(busqueda)) {
            row.style.display = '';
            visibles++;
        } else {
...
</html><?php /**PATH C:\Users\chris\OneDrive\Imágenes\Escritorio\Clinica Medica\clinica\resources\views/pacientes.blade.php ENDPATH**/ ?>