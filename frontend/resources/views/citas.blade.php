...
<aside class="sidebar">
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ $item['href'] }}" class="{{ $item['id'] === 'citas' ? 'active' : '' }}">
    ...
    <div class="content">
        <div class="page-header">
            <div class="page-header-left">
                <h1>{!! icon('calendar', 24) !!} Citas</h1>
                <p>Gestiona y visualiza todas las citas del consultorio.</p>
            </div>
            <button class="btn btn-primary" id="btnNuevaCita">{!! icon('plus', 16) !!} Nueva Cita</button>
        </div>

        <div class="stats-mini">
            <div class="stat-mini" onclick="activarFiltro('todos')" style="cursor:pointer">
                <div class="stat-mini-icon" style="background:#eef2ff;color:var(--blue);">{!! icon('calendar',20) !!}</div>
                <div><div class="stat-mini-val">{{ $total_citas }}</div><div class="stat-mini-lbl">Total citas</div></div>
            </div>
            <div class="stat-mini" onclick="activarFiltro('confirmada')" style="cursor:pointer">
                <div class="stat-mini-icon" style="background:#ecfdf5;color:var(--green);">{!! icon('check',20) !!}</div>
                <div><div class="stat-mini-val">{{ $confirmadas }}</div><div class="stat-mini-lbl">Confirmadas</div></div>
            </div>
            <div class="stat-mini" onclick="activarFiltro('pendiente')" style="cursor:pointer">
                <div class="stat-mini-icon" style="background:#fef9c3;color:#854d0e;">{!! icon('clock',20) !!}</div>
                <div><div class="stat-mini-val">{{ $pendientes }}</div><div class="stat-mini-lbl">Pendientes</div></div>
            </div>
            <div class="stat-mini" onclick="activarFiltro('cancelada')" style="cursor:pointer">
                <div class="stat-mini-icon" style="background:var(--red-light);color:var(--red);">{!! icon('x',20) !!}</div>
                <div><div class="stat-mini-val">{{ $canceladas }}</div><div class="stat-mini-lbl">Canceladas</div></div>
            </div>
        </div>
        ...
        <div class="table-card">
            <div class="table-card-header">
                ...
                <span class="table-info" id="tableInfo">Mostrando {{ $total_citas }} citas</span>
            </div>
            <div class="table-wrap">
                <table id="citasTable">
                    ...
                    <tbody id="citasBody">
                        ...
                        <tr data-estado="{{ $cita->estado }}" data-tipo="{{ $cita->tipo_cita }}" data-fecha="{{ $cita->fecha }}" data-nombre="{{ strtolower($cita->pac_nombre.' '.$cita->pac_apellido) }}" data-motivo="{{ strtolower($cita->motivo) }}" data-id="{{ $cita->id_cita }}">
                            <td>
                                <div class="td-paciente">
                                    <div class="td-avatar" style="background:{{ tipo_bg($cita->tipo_cita) }};color:{{ tipo_color($cita->tipo_cita) }};">
                                        {{ mb_strtoupper(mb_substr($cita->pac_nombre,0,1)) }}{{ mb_strtoupper(mb_substr($cita->pac_apellido,0,1)) }}
                                    </div>
                                    <div>
                                        <div class="td-nombre">{{ $cita->pac_nombre }} {{ $cita->pac_apellido }}</div>
                                        <div class="td-edad">{{ $cita->edad }} años · {{ $cita->telefono }}</div>
                                    </div>
                                </div>
                            </td>
                            <td>{{ date('d/m/Y', strtotime($cita->fecha)) }}</td>
                            <td><strong>{{ date('H:i', strtotime($cita->hora)) }}</strong></td>
                            <td>{{ $cita->motivo }}</td>
                            <td><span class="badge-tipo" style="background:{{ tipo_bg($cita->tipo_cita) }};color:{{ tipo_color($cita->tipo_cita) }};">{{ tipo_label($cita->tipo_cita) }}</span></td>
                            <td>{{ $cita->medico }}</td>
                            <td><span class="badge-estado-pill {{ estado_class($cita->estado) }}">{{ estado_label($cita->estado) }}</span></td>
                            <td>
                                <div class="td-actions">
                                    <button class="action-btn view" onclick="verCita({{ $cita->id_cita }})">{!! icon('eye',14) !!}</button>
                                    @if(in_array($cita->estado, ['programada','confirmada','pendiente']))
                                        <button class="action-btn edit" onclick="editarCita({{ $cita->id_cita }})">{!! icon('edit',14) !!}</button>
                                        <button class="action-btn del" onclick="cancelarCita({{ $cita->id_cita }})">{!! icon('trash',14) !!}</button>
                                    @else
                                        <button class="action-btn edit" disabled title="No se puede editar una cita {{ $cita->estado === 'completada' ? 'completada' : 'cancelada' }}" style="opacity:.35;cursor:not-allowed;">{!! icon('edit',14) !!}</button>
                                        <button class="action-btn del" disabled title="No se puede eliminar una cita {{ $cita->estado === 'completada' ? 'completada' : 'cancelada' }}" style="opacity:.35;cursor:not-allowed;">{!! icon('trash',14) !!}</button>
                                    @endif
                                </div>
                            </td>
                        </tr>
                ...
                <div id="emptyState" class="empty-table" style="{{ count($citas) === 0 ? '' : 'display:none;' }}">
                    {!! icon('search',48) !!}
            ...
            <div class="pagination">
                ...
                <div class="pagination-btns">
                    <button class="pag-btn">{!! icon('chevron-l',14) !!}</button>
                    <button class="pag-btn active">1</button>
                    <button class="pag-btn">{!! icon('chevron-r',14) !!}</button>
                </div>
            </div>
        </div>
    </div>
...
<div class="modal-overlay" id="modalNuevaCita">
    <div class="modal">
        <div class="modal-header">
            <h2>{!! icon('calendar',18) !!} <span id="modalTitle">Nueva Cita</span></h2>
            <button class="modal-close" id="closeModalCita">{!! icon('x',20) !!}</button>
        </div>
        <div class="modal-body">
                ...
                <div class="form-row full" style="margin-bottom:10px;">
                    <div class="form-group">
                        ...
                        <div class="search-wrap" style="margin-bottom:10px;">
                            {!! icon('search',14,'style="color:var(--text-soft)"') !!}
                            <input type="text" id="buscarPacienteInput" placeholder="Nombre del paciente...">
                        </div>
                        <div id="resultadosPacientes">
                            @foreach ($pacientes as $p)
                            <div class="patient-search-result" data-dui="{{ $p->dui }}" data-nombre="{{ $p->nombre }} {{ $p->apellido }}" onclick="seleccionarPaciente(this)">
                                <div class="psresult-av">{{ mb_strtoupper(mb_substr($p->nombre,0,1)) }}{{ mb_strtoupper(mb_substr($p->apellido,0,1)) }}</div>
                                <div class="psresult-info">
                                    <div class="psresult-name">{{ $p->nombre }} {{ $p->apellido }}</div>
                                    <div class="psresult-sub">{{ $p->edad }} años · {{ $p->telefono ?? 'Sin teléfono' }}</div>
                                </div>
                                <div class="psresult-check">{!! icon('check',16) !!}</div>
                            </div>
                            @endforeach
                        </div>
            ...
            <div class="form-row">
                <div class="form-group"><label>Fecha *</label><input type="date" id="cita_fecha" value="{{ date('Y-m-d') }}"></div>
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" id="guardarCita">{!! icon('check',16) !!} Guardar cita</button>
        </div>
    </div>
</div>
...
<div class="modal-overlay" id="modalVerCita">
    <div class="modal" style="max-width:500px;">
        <div class="modal-header">
            <h2>{!! icon('info',18) !!} Detalle de cita</h2>
            <button class="modal-close" onclick="cerrarModal('modalVerCita')">{!! icon('x',20) !!}</button>
        </div>
        <div class="modal-body" id="verCitaBody"></div>
        <div class="modal-footer">
            <button class="btn btn-ghost" onclick="cerrarModal('modalVerCita')">Cerrar</button>
            <button class="btn btn-outline" id="btnEditarDesdeVer">{!! icon('edit',14) !!} Editar</button>
        </div>
    </div>
</div>
...
<script>
const CITAS_DATA = @json($citas);
const PACIENTES_DATA = @json($pacientes);
const MEDICOS_DATA = @json($medicos);
...
// Validación solo letras para nombre/apellido en nuevo paciente (citas)
['np_nombres','np_apellidos'].forEach(function(fId) {
...
document.getElementById('guardarCita').addEventListener('click', async () => {
    ...
    if (activeTab === 'paciente-existente') {
        ...
        const soloLetrasRegex = /^[a-záéíóúüñÁÉÍÓÚÜÑ\s]+$/i;
        if (!soloLetrasRegex.test(nombres) || !soloLetrasRegex.test(apellidos)) {
