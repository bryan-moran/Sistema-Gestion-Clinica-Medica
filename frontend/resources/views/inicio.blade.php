...
<aside class="sidebar">
    <div class="sidebar-brand">
        <div class="brand-logo-sm">
            <img src="{{ asset('assets/img/logo.png') }}" alt="Logo Clínica Bienestar">
        </div>
    ...
    <nav class="sidebar-nav">
        ...
        <a href="{{ $item['href'] }}" class="{{ $item['id'] === 'inicio' ? 'active' : '' }}">
            {!! icon($item['icono'], 18) !!}
...
<div class="main">
            ...
            <div class="user-chip-wrap">
                ...
                <div class="profile-dropdown" id="profileDropdown">
                    ...
                    <div class="profile-dd-body">
                        <a href="{{ route('logout') }}" class="profile-dd-btn">
    ...
    <div class="content">
        <div class="page-header">
            <h1>{{ $saludo }}, {{ $nombre }}! <span>{{ $emoji_saludo }}</span></h1>
            <p>Aquí tienes un resumen de tu actividad de hoy.</p>
        </div>

        <div class="stats-row">
            <div class="stat-card" onclick="window.location='{{ route('citas') }}?fecha=hoy'" style="cursor:pointer">
                <div class="stat-top">
                ...
                <div class="stat-footer"><span>citas para hoy</span>{!! icon('chevron-r', 16) !!}</div>
            </div>
            <div class="stat-card" onclick="window.location='{{ route('citas') }}?estado=proximas'" style="cursor:pointer">
                <div class="stat-top">
                    ...
                    <div class="stat-texts">
                        ...
                        <div class="stat-value">{{ $citas_proximas }}</div>
                    </div>
                </div>
                <div class="stat-footer" style="color:var(--green)"><span>de mañana en adelante</span>{!! icon('chevron-r', 16) !!}</div>
            </div>
            <div class="stat-card urgencia" onclick="window.location='{{ route('citas') }}?tipo=urgencia'" style="cursor:pointer">
                <div class="stat-top">
                ...
                <div class="stat-footer"><span>registradas</span>{!! icon('chevron-r', 16) !!}</div>
            </div>
            <div class="stat-card registrar" onclick="window.location='{{ route('pacientes') }}'" style="cursor:pointer">
                <div class="stat-top">
                ...
                <div class="stat-footer"><span>registrados</span>{!! icon('chevron-r', 16) !!}</div>
            </div>
        </div>
        ...
        <div class="bottom-grid">
            <div class="card">
                <div class="card-header">
                    <h2>📅 Agenda de hoy</h2>
                    <a href="{{ route('citas') }}" class="link-blue">Ver agenda completa</a>
                </div>
                @if(count($agenda_hoy) === 0)
                <div class="empty-state">
                    {!! icon('calendar', 36) !!}
                    <p>No hay citas agendadas para hoy</p>
                </div>
                @else
                @foreach($agenda_hoy as $cita)
                <div class="actividad-item">
                    <div class="act-icon" style="background:#eef2ff;color:#1a56db;">{!! icon('clock', 16) !!}</div>
                    <div class="act-info">
                        <div class="act-titulo">{{ $cita->pac_nombre }} {{ $cita->pac_apellido }}</div>
                        <div class="act-detalle">{{ $cita->motivo }} · {{ $cita->medico }}</div>
                    </div>
                    <div class="act-tiempo">{{ substr($cita->hora, 0, 5) }}</div>
                </div>
                @endforeach
                @endif
                <a href="{{ route('citas') }}" class="ver-todas-btn">{!! icon('calendar', 16) !!} Ver todas las citas</a>
            </div>

            <div class="card">
                <div class="card-header">
                    <h2>{!! icon('calendar', 16) !!} &nbsp;Calendario</h2>
                </div>
                <div class="cal-header">
                    <button class="cal-nav">{!! icon('chevron-l', 16) !!}</button>
                    <span class="cal-month">{{ $nombre_meses[$mes_actual] }} {{ $anio_actual }}</span>
                    <button class="cal-nav">{!! icon('chevron-r', 16) !!}</button>
                </div>
                <div class="cal-grid">
            ...
            <div class="card actividad-card">
                ...
                <div class="actividad-item">
                    <div class="act-icon" style="background:#ecfdf5;color:#059669;">{!! icon('check', 16) !!}</div>
                    <div class="act-info">
                        <div class="act-titulo">{{ $act->pac_nombre }} {{ $act->pac_apellido }}</div>
                        <div class="act-detalle">{{ $act->motivo }}</div>
                    </div>
                    <div class="act-tiempo">{{ date('d/m', strtotime($act->fecha)) }}</div>
                </div>
                @endforeach
                @endif
            </div>
        ...
        <div class="accesos-grid">
            <button class="acceso-btn" id="btnAbrirCitaRapida">
                <div class="acceso-icon" style="background:#eef2ff;color:#1a56db;">{!! icon('calendar', 22) !!}</div>
                <span class="acceso-label">Nueva cita</span>
            </button>
            <button class="acceso-btn" id="btnRegistrarPaciente">
                <div class="acceso-icon" style="background:#ecfdf5;color:#059669;">{!! icon('plus-user', 22) !!}</div>
                <span class="acceso-label">Registrar paciente</span>
            </button>
            <a href="{{ route('pacientes') }}" class="acceso-btn">
                <div class="acceso-icon" style="background:#ecfeff;color:#0891b2;">{!! icon('search', 22) !!}</div>
            ...
            <button class="acceso-btn" disabled title="Módulo en desarrollo" style="opacity:.45;cursor:not-allowed;">
                <div class="acceso-icon" style="background:#f5f3ff;color:#7c3aed;">{!! icon('file-text', 22) !!}</div>
                <span class="acceso-label">Receta rápida</span>
            </button>
            <button class="acceso-btn" disabled title="Módulo en desarrollo" style="opacity:.45;cursor:not-allowed;">
                <div class="acceso-icon" style="background:#fffbeb;color:#d97706;">{!! icon('note', 22) !!}</div>
    ...
    <footer class="dash-footer">
        {!! icon('shield', 13) !!} &nbsp;&copy; {{ date('Y') }} Clínica Bienestar. Todos los derechos reservados.
    </footer>
</div>
</div>
...
<div class="modal-overlay" id="modalCitaRapida">
    <div class="modal">
        <div class="modal-header">
            <h2>{!! icon('calendar', 18) !!} Nueva Cita Rápida</h2>
            <button class="modal-close" id="cerrarModalCita">{!! icon('x', 20) !!}</button>
        </div>
        <div class="modal-body">
            ...
            <div class="form-row">
                <div class="form-group">
                    <label>Fecha *</label>
                    <input type="date" id="cr_fecha" value="{{ date('Y-m-d') }}" required>
                </div>
                <div class="form-group">
                    <label>Hora *</label>
...
<div class="modal-overlay" id="modalRegistrarPaciente">
    <div class="modal">
        <div class="modal-header">
            <h2>{!! icon('plus-user', 18) !!} Registrar Nuevo Paciente</h2>
            <button class="modal-close" id="cerrarModalPaciente">{!! icon('x', 20) !!}</button>
        </div>
        <div class="modal-body">
            <div class="form-row">
                <div class="form-group">
        ...
        <div class="modal-footer">
            ...
            <button class="btn btn-primary" id="guardarPacienteRapido">
                {!! icon('check', 16) !!} Guardar paciente
            </button>
        </div>
    </div>
</div>
...
document.getElementById('guardarCitaRapida').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch('{{ route("citas.rapida") }}', {
            method: 'POST',
            headers: {
...
document.getElementById('guardarPacienteRapido').addEventListener('click', async function() {
    ...
    try {
        const response = await fetch('{{ route("paciente.guardar") }}', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
